...
function parseQty(text, defaultUnit) {
  ...
  if (!unitRaw) return { qty, unit: defaultUnit, assumed: true };
...
bot.action(/^PROD_(\d+)$/, (ctx) => {
  ...
  ctx.reply(
    `Qty for ${chosen.name}?\n\n` +
    `Send just a number for ${chosen.unit}, or add a unit — for example "10 pcs" or "10 sqft".`
  );
});
...
bot.on('text', async (ctx) => {
  ...
  if (s.step === 'ENTER_QTY') {
    ...
    let note = '';
    if (parsed.assumed) {
      note = `\n(Recorded in ${productUnit}, the product's own unit.)`;
    } else if (parsed.unit !== productUnit) {
      note = `\n(Note: you gave ${parsed.unit}, but this product is normally ${productUnit}.)`;
    }
